...
from Decorators import MyTime, MyDecimal
...
class Transaction(ComparableMixin, Base):
    ...
    _trans_id = Column(Integer, primary_key=True)
    _acc_id = Column(Integer, ForeignKey("account._account_number"))
    _amt = Column(MyDecimal(length=100))
    _date = Column(MyTime(length=10))
    _exempt = Column(Integer)
    ...
    def is_withdrawal(self):
        "Returns whether the transaction is a withdrawal (negative amount)."
        return self._amt < 0
