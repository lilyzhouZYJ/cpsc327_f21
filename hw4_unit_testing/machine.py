...
class Enigma():
    ...
    def __repr__(self):
        return f"Keyboard <-> Plugboard <->  Rotor {self.rotor_order[0]} <-> Rotor  {self.rotor_order[1]} <-> Rotor  {self.rotor_order[2]} <-> Reflector \nKey:  + {self.key}"
    ...
    def encipher(self, message):
        ...
        if bool(re.compile(r'[^a-zA-Z ]').search(message)):
            raise ValueError('Please provide a string containing only the characters a-zA-Z and spaces.')
